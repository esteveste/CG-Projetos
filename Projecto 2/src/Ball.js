...
let ball_geo = [this.radius, 32, 32];
...
var Ball = function () {
    ...
    var x = Math.random() * ((width/2)-this.radius-0.5 - (-width/2 + this.radius+0.5)) + (-width/2 + this.radius+0.5);
    var z = Math.random() * ((depth/2)-this.radius-0.5 - (-depth/2 + this.radius+0.5)) + (-depth/2 + this.radius+0.5);
